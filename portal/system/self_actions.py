...
class SELFService(object):
    def __init__(self, app_id, token_name):
        ...
        if not self.application.need_auth:
            raise RedirectionNeededError("Application '{}' does not need authentication".format(self.application.name), self.application.get_redirect_uri())
    ...
    def get_username_by_email(self, backend, fallback_backends, email):
        ...
        try:
            ...
            logger.info("SELF::get_user_by_email: User '{}' successfully found on backend '{}'".format(result['user'], result['backend'].repository))
            self.backend_id = str(backend.id) # TODO : Needed ?????
            return result

        except Exception as e:
            logger.error("SELF::get_user_by_email: Failed to find email '{}' on primary backend '{}' : '{}'".format(email, str(backend), str(e)))
            logger.exception(e)
            for fallback_backend in fallback_backends:
                try:
                    ...
                    logger.info("SELF::get_user_by_email: User '{}' successfully found on backend '{}'".format(result['user'], result['backend']))
    ...
    def set_authentication_params(self, repo, authentication_results ,username):
        if authentication_results:
            ...
            raise AuthenticationError("SELF::authenticate: Authentication result is empty for username '{}'".format(username))


    def authenticate_on_backend(self, backend, username, password):

        if isinstance(backend.get_backend(), MongoEngineBackend):
            authentication_results = self.set_authentication_params(backend, backend.get_backend().authenticate(username, password), username)
        else:
            authentication_results = backend.get_backend().authenticate(username, password, acls=self.application.access_mode, logger=logger)

        logger.info("AUTH::authenticate: User '{}' successfully authenticated on backend '{}'".format(username, backend))
    ...
    def retrieve_credentials(self, request):
        ...
        self.username = self.redis_portal_session.keys.get('login_' + str(self.application.getAuthBackend().id)) or (([self.redis_portal_session.keys.get('login_' + str(backend_fallback.id)) for backend_fallback in self.application.getAuthBackendFallback() if self.redis_portal_session.keys.get('login_' + str(backend_fallback.id))] or [None])[0])
    ...
    def perform_action(self):
        ...
        auth_apps = Application.objects(Query).only('name', 'public_name', 'public_dir', 'id', 'type', 'listeners', 'need_auth')
    ...
    def ask_credentials_response(self, request, action, error_msg):
        return self_ask_passwords(request, self.application, self.token_name, request.GET.get("rdm", None) or request.POST.get('rdm', None), action, error_msg)
...
class SELFServiceChange(SELFService):
        ...
        if rdm:
            ...
            if self.redis_portal_session.keys.get('login_' + str(self.application.getAuthBackend().id)):
                try:
                    ...
                    logger.error("Seems to have wrong old password on backend : '{}', exception details : "+self.application.getAuthBackend().repo_name)
                    logger.exception(e)
            if not user_infos:
    ...
    def perform_action(self, request, old_password):
        ...
        if not rdm:
            ...
            if not self.redis_portal_session.getAutologonPassword(str(saved_app.id), str(self.backend.id), self.username):
                raise AuthenticationError("Wrong old password")
        else:
        ...
        if not (len (new_passwd) >= min_len and upper_case >= min_upper and lower_case >= min_lower and number >= min_number and symbol >= min_symbol):
        ...
        if issubclass(self.backend.__class__, Backend):
            self.backend.change_password(self.username, old_password, new_passwd, krb5_service=self.application.app_krb_service)
            ...
            self.backend.get_backend().change_password(self.username, old_password, new_passwd, krb5_service=self.application.app_krb_service)
        ...
        if not rdm:
            if self.redis_portal_session.setAutologonPassword(str(saved_app.id), str(saved_app.name), str(self.backend.id), self.username, old_password, new_passwd) is None:
...
class SELFServiceLost(SELFService):
    ...
    def retrieve_credentials(self, request):
        ...
        if not re_match(r"^[A-Za-z0-9\.\+_-]+@[A-Za-z0-9\._-]+\.[a-zA-Z]+$", email):
            raise PasswordMatchError("Email address not valid.")  # Do NOT raise anything else

        """ Try catch here to prevent error in form and blind extraction of repo """
        try:
            user_infos    = self.get_username_by_email(self.application.getAuthBackend(), self.application.getAuthBackendFallback(), email)
        except Exception as e:
            logger.error("SELF::Lost: Error trying to get email '{}': {}".format(email, e))
            user_infos = {}

        self.username = user_infos.get('user')

        return email


    def send_lost_mail(self, request, email):
        """ Method used to send a mail with a temporary key """

        """ If not username, do not raise to not indicate the user its mail do not exists
        and prevent blind database extraction """
        if self.username:
            ...
            msg['subject'] = env.get_template("portal_%s_email_subject.conf" % (str(self.application.template.id))).render({'app': obj})
            email_body = env.get_template("portal_%s_email_body.conf" % (str(self.application.template.id))).render({'resetLink': reset_link, 'app': obj})
            ...
            try:
                ...
                logger.info("SELF::Lost: Email successfully sent to '{}'".format(email))
            except Exception as e:
        ...
        return "Mail successfully sent to '{}'".format(escape(email))
...
class SELFServiceLogout(SELFService):
    ...
    def perform_action(self, request, app_cookie_list):
        ...
        return "{} successfully disconnected".format(escape(self.username))
