...
from portal.system.exceptions import TokenNotFoundError, REDISWriteError
...
class REDISAppSession(REDISSession):
    ...
    def __init__(self, redis_handler, **kwargs):
        ...
        self.mod_vulture_token = None
        app_cookie = None
        if token:
            self.mod_vulture_token = token
            app_cookie             = redis_handler.get(token)

        if not app_cookie and cookie:
            app_cookie = cookie

        if app_cookie is None:
            raise TokenNotFoundError("REDISSession: No session found for token " + str(token))

        self.default_timeout   = default_timeout
        super().__init__(redis_handler, app_cookie)

        try:
            assert( self.keys['authenticated'] )
        except (AssertionError, KeyError, TypeError) as e:
            """ Delete useless Token """
            self.handler.delete(token)

            """ Delete bad cookie Session """
            self.handler.delete(app_cookie)

            raise TokenNotFoundError("REDISAppSession: No user session found for token '{}'".format(self.key))


    def retrieve_captcha(self):
        return self.handler.hget(self.key, 'captcha')
    ...
    def is_double_authenticated(self):
        return (self.handler.hget(self.key, 'doubleauthenticated') == "1")


    def get_login(self):
        return self.handler.hget(self.key, 'login')
    ...
    def setHeader(self, headers):
        ...
        self.handler.hset(self.key, 'headers', headers)
        return True
    ...
    def setKrb5Infos(self, username, krb5service, backend_id):
        ...
        krb5ccname               = "FILE:/tmp/krb5cc_" + sha1("{}{}".format(backend_id, username).encode('utf8')).hexdigest()
    ...
    def register_authentication(self, app_id, username, timeout):
        ...
        self.keys['login']          = username
        self.keys['application_id'] = app_id
        self.keys['otp_retries']    = 0

        # Delete useless token
        if self.mod_vulture_token:
            self.handler.delete(self.mod_vulture_token)
    ...
    def register_doubleauthentication(self):
        self.keys['doubleauthenticated'] = "1"
        self.handler.hset(str(self.key), 'doubleauthenticated', '1')
    ...
    def register_captcha(self):
        ...
        self.handler.hset(self.key, 'captcha', secret_key)
...
class REDISPortalSession(REDISSession):
    ...
    def authenticated_backend(self, backend_id):
        return self.handler.hget(self.key, str(backend_id))


    def get_oauth2_token(self, backend_id):
        return self.handler.hget(self.key, 'oauth2_'+str(backend_id)) or ""
    ...
    def setAutologonPassword(self, app_id, app_name, backend_id, username, old_password, new_password):
        ...
        if self.getAutologonPassword(app_id, backend_id, username) == old_password:
            # Update password
            pwd = SSOProfile()
            p = pwd.set_data(str(app_id), app_name, str(backend_id), BaseAbstractRepository.search_repository(backend_id).repo_name, username, 'vlt_autologon_password', new_password)
            if p:
    ...
    def register_authentication(self, app_id, app_name, backend_id, url, dbauthentication_required, username, password, oauth2_token, app_token, authentication_datas, timeout):
        ...
        if password:
            ...
            pwd = SSOProfile()
            p = pwd.set_data(app_id, app_name, backend_id, BaseAbstractRepository.search_repository(backend_id).repo_name, username, 'vlt_autologon_password', password)
            self.keys['password_'+backend_id] = p

        if not self.write_in_redis(timeout or self.default_timeout):
            raise REDISWriteError("REDISPortalSession::register_authentication: Unable to write authentication infos in REDIS")

        return self.key


    def register_doubleauthentication(self, app_id):
        backend_id = self.keys['backend_'+app_id]
        self.keys[backend_id] = 1
        self.handler.hset(self.key, backend_id, "1")


    def register_sso(self, app_token, timeout, backend_id, app_id, url, username, oauth2_token):
        self.keys[backend_id]         = 1
        self.keys['url_'+app_id]          = url
        self.keys['backend_'+app_id]      = backend_id
        self.keys['login_'+backend_id]    = username
...
class REDISBase(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            self.r = Redis(unix_socket_path='/var/db/redis/redis.sock', db=0, decode_responses=True)
            self.r.info()
        except Exception as e:
