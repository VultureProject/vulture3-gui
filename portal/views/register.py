...
from vulture_toolkit.auth.exceptions import UserNotFound
...
class Registration(object):
    def __init__(self, app_id, token_name):
        ...
        if not self.application.enable_registration:
            raise RedirectionNeededError("Registration is not enabled in the application '{}'".format(self.application.name), self.application.get_redirect_uri())
...
class STEP1Registration(Registration):
    ...
    def retrieve_credentials(self, request, token):
        ...
        try:
            ...
            if not user_infos:
                ...
                raise UserAlreadyExistsError("REGISTER::search_user: User '{}' already found on repository '{}' with email '{}'".format(user_infos['user'], backend.repo_name, email))
    ...
    def perform_action(self, request, email, token):
        ...
        reset_link = self.application.get_redirect_uri() + str(self.token_name) + '/register?registrk=' + reset_key

        msg = MIMEMultipart('alternative')
        email_from  = self.application.template.email_register_from
        msg['From'] = email_from
        msg['To']   = email
        ...
        msg['subject'] = env.get_template("portal_%s_email_register_subject.conf" % (str(self.application.template.id))).render(
            {'app': obj})
        email_body = env.get_template("portal_%s_email_register_body.conf" % (str(self.application.template.id))).render(
    ...
    def message_response(self, message):
        style = render_stylesheet('/{}/templates/portal_{}.css'.format(str(self.token_name), str(self.application.template.id)))
        link_redirect = self.application.get_redirect_uri() + str(self.token_name) + "/register"
        return render_to_response("portal_%s_html_message.conf" % str(self.application.template.id),
...
class STEP2Registration(Registration):
    ...
    def perform_action(self, request, email, token):
        ...
        self.backend.get_backend().add_new_user(username, password1, email, phone,
                                                group=self.application.group_registration,
                                                update_group=self.application.update_group_registration)
    ...
    def ask_credentials_response(self, request, token, message=None):
        captcha_key = get_random_string(32)
        captcha     = self.generate_captcha(captcha_key)
        return register_ask2(request, self.application, self.token_name, token, self.backend.get_phone_column() != "", captcha_key, captcha, message)


    def message_response(self, message):
        style = render_stylesheet('/{}/templates/portal_{}.css'.format(str(self.token_name), str(self.application.template.id)))
        link_redirect = self.application.get_redirect_uri()
        return render_to_response("portal_%s_html_message.conf" % str(self.application.template.id),
                                  {'style': style, 'link_redirect': link_redirect, 'message': message})
...
def registration(request, token_name, proxy_app_id=None):
    ...
    # Redis connection error
        ...
        logger.error("REGISTER::init: Application with id '{}' not found".format(proxy_app_id))
    ...
    try:
        ...
        logger.error("REGISTER::Error: An unknown error occured while registering : ")
        logger.exception(e)
        return registration.ask_credentials_response(request, registrk, "An unknown error occured <br> <b> Please contact your administrator </b>")
