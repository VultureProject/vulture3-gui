...
from gui.models.repository_settings import SyslogRepository, MongoDBRepository
from gui.models.system_settings import Cluster
from gui.models.application_settings import Application
from vulture_toolkit.system.logrotate import Logrotate
from vulture_toolkit.system.syslog import Rsyslogd

logger = logging.getLogger('cluster')

def check_status(request):
    """ Function called when an application or log profile is edited. It checks
     if rsyslogd configuration has changed. If changes are detected, it write
     new configuration files and restart rsyslogd services
    ...
    data_repo_apps = list()
    learning_apps = [] # Application.objects.filter(learning=True)
        ...
        if app.log_custom in data_repo_logs:
            ...
            if app.log_custom.repository.is_internal:
                ...
                if app.log_custom.repository.db_user and app.log_custom.repository.db_password:
                    app.uristr="mongodb://"+app.log_custom.repository.db_user+":"+app.log_custom.repository.db_password+"@"+app.log_custom.repository.db_host+':'+str(app.log_custom.repository.db_port)+'/'
                else:
                    app.uristr="mongodb://"+app.log_custom.repository.db_host+':'+str(app.log_custom.repository.db_port)+'/'
        ...
        if node.system_settings.pf_settings.repository_type == 'data':
            ...
            if pf_settings.is_internal:
                ...
                if pf_settings.db_user and pf_settings.db_password:
                    pf_settings.uristr="mongodb://"+pf_settings.db_user+":"+pf_settings.db_password+"@"+pf_settings.db_host+':'+str(pf_settings.db_port)+'/'
                else:
    ...
    """ Now dealing with Logrotate configuration """
    logrotate = Logrotate()
    new_conf = logrotate.conf_has_changed(conf_logrotate)
    if new_conf:
        result['logrotate_changed'] = 'changed'
        logrotate.write_configuration(None, None, new_conf)

    """ Now dealing with rsyslogd service """
    rsyslogd = Rsyslogd()
    new_conf = rsyslogd.conf_has_changed(syslog_conf)
    if new_conf:
        result['rsyslogd_changed'] = 'changed'
        if rsyslogd.write_configuration(None, None, new_conf):
            status = rsyslogd.restart_service()
            result['status_rsyslogd'] = status
