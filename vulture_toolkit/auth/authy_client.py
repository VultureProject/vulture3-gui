...
class AuthyClient(BaseAuth):
        ...
        if self.type == 'onetouch':
            if verification.ok() and verification.status():
                if verification.content.get('approval_request', {}).get('status') == 'approved':
                    ...
                    raise OTPError("The OneTouch request is not yet approved, status : {}".format(verification.content.get('approval_request', {}).get('status')))

        # If we arrive here : verification.ok() returned False ; Authentication failure
        raise AuthenticationError("Authy authentication error while verificating token : {}".format(verification.errors()))
    ...
    def register_authentication(self, user_mail, user_phone):
        ...
        if not user.ok():
            logger.error("AUTHY_CLIENT::register_authentication: Creation of new user in authy with mail:'{}', phone:'{}' failure : {}".format(user_mail, user_phone, user.errors()))
            raise RegisterAuthenticationError("Unable to register Authy user with mail:'{}', phone:'{}' : {}".format(user_mail, user_phone, user.errors()))
        ...
        if not response.ok():
            logger.error("AUTHY_CLIENT::register_authentication: Unable to send request to '{}': {}".format(user_phone, response.errors()))
            raise RegisterAuthenticationError("Unable to send Authy request to '{}' : {}".format(user_phone, response.errors()))
