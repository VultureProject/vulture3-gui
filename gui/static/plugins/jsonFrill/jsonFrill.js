...
;(function ( $, window, document, undefined ) {
    $.fn.jsonFrill = function(options, jsonSource) {
        ...
        function processPrimitive(key, value, type) {
            if (key === 'info')
                return ""
            return '<div class="jf-prop jf-item detail_info' + collapsedClass + ' " >' + getKey(key) + seperator + '<span class="jf-value value jf-' + type +'">' + value + '</span></div>';
        }
        ...
        function getKey(key, jfClass) {
            if(jfClass) {
                return '<span class="'+jfClass+'">' + SPACES + '<span class="jf-key jf-collapse"><code class="key">' + key + '</code></span></span>';
            }
            return '<span class="jf-key">' + SPACES + "<code class='key'>" + key + '</code></span>';
        }
        ...
        function bindings() {
            ...
            $('div#jf-toolbar').on('click', "button", function(){
                $(this).text() === 'Tout Dissocier' ? jfShow($('div.jf-collapsed'), elements.collapsibleKeys)
        ...
        function toolBar(collapseAll) {
            return "<div id='jf-toolbar'>" +
                        "<button class='btn btn-primary btn-sm' id='jf-collapse-all'><i class='fa fa-compress'></i>&nbsp;Collapse</button>" +
                        "<button class='btn btn-primary btn-sm' id='jf-expand-all'><i class='fa fa-expand'></i>&nbsp;Expand</button>" +
                    "</div>";
        }
